...
const ColorPizza = ({
  ...
  const [paletteTitle, setPaletteTitle] = useState(
    (defaultkv as any)[hexValue] != undefined
      ? (defaultkv as any)[hexValue]
      : ".isloading"
  );
...
const Spinner = ({ color }: { color: string }) => {
  ...
  const complement = chroma([m - c[0], m - c[1], m - c[2]]).luminance(
    1 - col.luminance() - 0.25
  );
  return (
    <div role="status" className="">
      <div
        style={{
          borderColor: complement.hex(),
          borderTopColor: col.hex(),
          animation: "spin 1s cubic-bezier(.34,.6,.85,.74) infinite",
        }}
